...
`toyDisplay.fillLight` is the safer per-scene sprite brightening path for non-concert scenes. It duplicates the same transparent toy PNG as a low-opacity additive layer, so the lift is clipped to the sprite alpha instead of drawing a rectangular light card behind the toy. Tune `color`, `opacity`, and optional `scale` per scene; keep `sence5` on its dedicated `keyLight` baseline unless a new visual pass explicitly accepts a change there.
